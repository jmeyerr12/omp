...
#   OMP_ONLY_INPUTS="inputs/input_105.txt ..."   # Parte B (opcional)
...
OMP_ONLY_INPUTS=${OMP_ONLY_INPUTS:-"inputs/input_140.txt inputs/input_150.txt inputs/input_160.txt inputs/input_170.txt 
...
# Extrai o ÚLTIMO token numérico da saída (cobre stdout/stderr mistos)
extract_time() {
  awk '{
...
# Expande padrões em uma lista ordenada "naturalmente" (70 < 100)
expand_and_sort() {
  local patterns=("$@")
  local tmp=()
  shopt -s nullglob
...
if [ "$DO_BASE" -eq 1 ]; then
  ...
  for in_file in "${files_base[@]}"; do
    ...
    # Paralelo: lê o tempo impresso pelo binário OMP
    for t in $THREADS; do
      omp_list=""
      for ((i=1;i<=REPS;i++)); do
      ...
      echo "  T=$t  omp  mean=$omp_mean  sd=$omp_sd  | speedup=$speedup  eff=$efficiency"
    done
  done
...
if [ "$DO_OMP_EXTRA" -eq 1 ] && [ -n "$OMP_ONLY_INPUTS" ]; then
  ...
  for in_file in "${files_omp_only[@]}"; do
    ...
    for t in $THREADS; do
      ...
      echo "  T=$t  omp  mean=$omp_mean  sd=$omp_sd  thrpt=$thrpt"
    done
  done
